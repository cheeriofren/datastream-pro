...
def test_get_user_me(test_token):
    """Test endpoint mendapatkan data user yang sedang login"""
    response = client.get(
        "/users/me",
...
def test_create_dataset(test_token, test_user):
    """Test endpoint pembuatan dataset"""
    dataset_data = {
        "name": "New Dataset",
        "source": "new_source",
...
def test_get_datasets(test_token, test_dataset):
    """Test endpoint mendapatkan daftar dataset"""
    response = client.get(
        "/datasets/",
        headers={"Authorization": f"Bearer {test_token}"}
    )
...
def test_get_dataset(test_token, test_dataset):
    """Test endpoint mendapatkan detail dataset"""
    response = client.get(
...
def test_create_analysis(test_token, test_dataset):
    """Test endpoint pembuatan analisis"""
    analysis_data = {
...
def test_get_analyses(test_token, test_dataset):
    """Test endpoint mendapatkan daftar analisis"""
    response = client.get(
        "/analyses/",
        headers={"Authorization": f"Bearer {test_token}"}
    )
...
def test_get_analysis(test_token, test_dataset):
    """Test endpoint mendapatkan detail analisis"""
    # Buat analisis test
    analysis = Analysis(
        dataset_id=test_dataset.id,
        analysis_type="time_series",
...
def test_rate_limit(test_token):
    """Test rate limiting"""
    # Buat request berulang
    for _ in range(100):  # Asumsi rate limit 60 request per menit
        response = client.get(
            "/users/me",
            headers={"Authorization": f"Bearer {test_token}"}
        )
...
def test_validation_error():
    ...
    user_data = {
        ...
        "email": "invalid_email",  # Email tidak valid
        "password": "short"  # Password terlalu pendek
    }
    response = client.post("/users/", json=user_data)
    assert response.status_code == 422

def test_not_found(test_token):
    """Test endpoint tidak ditemukan"""
    response = client.get(
        "/datasets/999",  # Dataset ID tidak ada
        headers={"Authorization": f"Bearer {test_token}"}
    )
